...
class _PagoFormState extends State<PagoForm> {
  ...
  TextEditingController _valorDolarController = TextEditingController();
  List<String> _nombrePeajes = [];
  Map<String, Map<String, double>> _peajesValores = {};
  double _valorDolar = 0.0;
  ...
  Future<void> _fetchValorDolar() async {
    final url = Uri.parse('https://www.datos.gov.co/resource/mcec-87by.json');
    try {
      final response = await http.get(url);
      if (response.statusCode == 200) {
        final List<dynamic> data = json.decode(response.body);
        setState(() {
          _valorDolar = double.tryParse(data.first['valor'].toString()) ?? 0.0;
          _calcularValorEnDolares();
        });
      } else {
        throw Exception('Failed to load valor del dólar');
      }
    } catch (e) {
      print('Error fetching valor del dólar: $e');
    }
  }

  // Dentro de _calcularValorEnDolares en _PagoFormState
void _calcularValorEnDolares() {
  if (_valorDolar > 0) {
    final valorEnDolares = _editedPago.valor / _valorDolar;
    _valorDolarController.text = valorEnDolares.toStringAsFixed(2);
  }
}
    ...
    if (_formKey.currentState!.validate()) {
      ...
      try {
        final url = _editedPago.id == null
            ? Uri.parse(
                'http://santiagoflorezzz-001-site1.ktempurl.com/api/Pagos')
            : Uri.parse(
                'http://santiagoflorezzz-001-site1.ktempurl.com/api/Pagos/${_editedPago.id}');
        ...
        child: Form(
          ...
          child: ListView(
            children: [
              ...
              TextFormField(
                readOnly: true,
                decoration: InputDecoration(labelText: 'Valor en COP'),
                controller:
              ...
              TextFormField(
                readOnly: true,
                decoration: InputDecoration(labelText: 'Valor en USD'),
